    ...
    *,
    ...
    while elapsed < duration_s - 1e-9:
        ...
        if closed_boundary:
            ...
            qx[:, 0] = np.where(active[:, 0], -source_depth[:, 0] * edge_speed[:, 0], 0.0)
            qx[:, -1] = np.where(active[:, -1], source_depth[:, -1] * edge_speed[:, -1], 0.0)
            qz[0, :] = np.where(active[0, :], -source_depth[0, :] * edge_speed[0, :], 0.0)
            qz[-1, :] = np.where(active[-1, :], source_depth[-1, :] * edge_speed[-1, :], 0.0)
...
def flood_preview(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    infiltration_rate = request.infiltration_mm_h / 3_600_000.0
    source_rate = np.where(terrain["active"], max(0.0, rain_rate - infiltration_rate), 0.0)
    ...
    mass_balance_error_pct = (
        abs(final_volume - source_volume) / source_volume * 100.0
    ...
    return {
        "model": {
            ...
            "boundary": "closed_user_drawn_box_no_surface_outflow",
